...
### Two-Phase Approach
1. **Generate phase** (`options/generate.js`, `generate-parallel.js`): Pre-filter valid game orderings per round based on hard constraints (no triple-headers, max idle slots, max span). For 8 teams × 3 games/night × 4 weeks, this reduces 12!⁴ (5.2e34) to 96⁴ (85M) combinations by eliminating unacceptable per-round schedules.
...
4. Update import line in `evaluate.js` to reference new options file

### Changing Scoring Criteria
Edit `painMultipliers` object in `evaluate.js` (lines ~15-28):
...
- **`evaluate.js`**: Main evaluation logic, scoring functions, entry point for single-threaded runs
- **`evaluate-parallel.js`**: Worker-based parallelization with progress reporting and resume capability
